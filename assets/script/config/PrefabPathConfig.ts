

//此文件为自动生成，不要手动修改


export enum PrefabPath {
    window_common_SysLoading = "prefab/window/common/SysLoading",
    window_common_SysTipView = "prefab/window/common/SysTipView",
    window_common_SysMsgBox = "prefab/window/common/SysMsgBox",
    window_web_LayerMain = "prefab/window/web/LayerMain",
    window_web_LayerWin = "prefab/window/web/LayerWin",
    window_web_LayerRecord = "prefab/window/web/LayerRecord",
    window_web_LayerBuyRecord = "prefab/window/web/LayerBuyRecord",
    window_web_LayerTip = "prefab/window/web/LayerTip",
    window_web_LayerRule = "prefab/window/web/LayerRule",
    window_web_LayerShop = "prefab/window/web/LayerShop",
    window_web_LayerShopTip = "prefab/window/web/LayerShopTip",
    window_web_LayerBuy = "prefab/window/web/LayerBuy",

}

