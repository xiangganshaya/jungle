...
let Base64Util = (function (global) {
    'use strict';
    ...
    let encodeURI = function (u) { return encode(u, true) };
    // decoder stuff
    ...
    let _atob = global.atob ? function (a) {
...
export default class Base64 {
    private static uint8ToString = function (u8a) {
        var CHUNK_SZ = 0x8000;
        var c = [];
    ...
    public static atob(data: any) {
        return Base64Util.atob(data);
    }

    public static btoa(data: any) {
        let u8a = new Uint8Array(data);
        let value='';
        try {
            value = this.uint8ToString(u8a);
        } catch (e) {
            // fix windows xp chrome 下首次执行 String.fromCharCode.apply报错问题
            value = this.uint8ToString(u8a);
        }
        return Base64Util.btoa(value);
    }
}
